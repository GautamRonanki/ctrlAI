...
Reads and manages files in Google Drive.
...
async def list_files(
    ...
    if not check_scope_permission(agent_name, "drive.readonly"):
        return {"error": f"Permission denied: {agent_name} does not have drive.readonly scope"}
    ...
    if response.status_code != 200:
        return {"error": f"Drive API error: {response.status_code}", "details": response.json()}
...
async def search_files(
    google_token: str, query: str, max_results: int = 10, agent_name: str = "drive_agent"
) -> dict:
    ...
    if response.status_code != 200:
        return {"error": f"Drive search failed: {response.status_code}", "details": response.json()}
...
async def delete_file(
    ...
    """
    if not check_scope_permission(agent_name, "drive.file"):
        return {"error": f"Permission denied: {agent_name} does not have drive.file scope"}

    if is_high_stakes(agent_name, "delete_file"):
        log_audit(
    ...
    if response.status_code != 204:
        return {"error": f"Delete failed: {response.status_code}", "details": response.json() if response.text else {}}
