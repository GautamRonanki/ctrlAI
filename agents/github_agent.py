...
Reads repos and manages issues via the GitHub API.
...
async def list_repos(
    ...
    if not check_scope_permission(agent_name, "repo"):
        return {"error": f"Permission denied: {agent_name} does not have repo scope"}
    ...
    if response.status_code != 200:
        return {"error": f"GitHub API error: {response.status_code}", "details": response.json()}
...
async def list_issues(
    github_token: str, owner: str, repo: str, max_results: int = 10, agent_name: str = "github_agent"
) -> dict:
...
async def create_comment(
    ...
    if is_high_stakes(agent_name, "create_comment"):
        log_audit(
    ...
    if response.status_code != 201:
        return {"error": f"Comment failed: {response.status_code}", "details": response.json()}
    ...
    log_audit(
        ...
        action="create_comment",
        status="success",
        details={"repo": f"{owner}/{repo}", "issue": issue_number, "comment_id": result.get("id")},
    )

    return {"status": "commented", "comment_id": result.get("id"), "url": result.get("html_url", "")}
