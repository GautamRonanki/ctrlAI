...
def run_inter_agent_tests() -> list[dict]:
    ...
    for test in INTER_AGENT_TESTS:
        actual = check_inter_agent_permission(test["requesting"], test["target"], test["action"])
        passed = actual == test["expected"]
...
def format_eval_report(report: dict) -> str:
    ...
    lines.append(f"")
    lines.append(f"Overall: {summary.get('total_passed', 0)}/{summary.get('total_tests', 0)} passed ({summary.get('pass_rate', 0)}%)")
    lines.append(f"")

    for cat_name, cat_data in report.get("categories", {}).items():
        lines.append(f"{cat_name.replace('_', ' ').title()}: {cat_data['passed']}/{cat_data['total']} passed")
        for test in cat_data.get("tests", []):
            ...
            if not test.get("passed"):
                if "expected_agent" in test:
                    lines.append(f"      Expected: {test['expected_agent']}/{test['expected_action']}")
                    lines.append(f"      Got: {test.get('actual_agent', '?')}/{test.get('actual_action', '?')}")
        lines.append("")
